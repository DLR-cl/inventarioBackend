...
export class PrestamoRegularService {
  ...
  async create(
    createPrestamoRegular: CreatePrestamoRegularDto,
  ): Promise<responsePrestamoRegular> {
    try {
  ...
  public async findOne(id_prestamo: number) {
    try {
      ...
      throw new InternalServerErrorException(
        'Error interno al obtener un prestamo',
      );
    }
  }
  ...
  private async setTimeSancion(
    rut_estudiante: string,
  ): Promise<TiempoSancionDto> {
    const findSancion: sanciones[] =
      await this.databaseService.sanciones.findMany({
        where: {
  ...
  public async obtenerPrestamosActivos(page?: number, limit?: number) {
    try {
      ...
      throw new InternalServerErrorException(
        'Error interno al obtener los préstamos activos',
      );
    }
  }
  ...
  public async obtenerPrestamosFinalizados() {
    try {
      ...
      throw new InternalServerErrorException(
        'Error interno al obtener los prestamos finalizados',
      );
    }
  }
}
